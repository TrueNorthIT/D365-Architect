...
/// D365CLI_CLIENT_ID / D365CLI_AUTHORITY environment variables in that case.
/// </summary>
public sealed record AuthenticationOptions(string ClientId, string Authority)
{
    ...
    public static AuthenticationOptions FromEnvironment() => new(
        Environment.GetEnvironmentVariable("D365CLI_CLIENT_ID") ?? DefaultClientId,
        Environment.GetEnvironmentVariable("D365CLI_AUTHORITY") ?? DefaultAuthority);
}
